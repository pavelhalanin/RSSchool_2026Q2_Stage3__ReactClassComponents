...
import CardListLeftButton from './CardListLeftButton';
...
describe('CardListLeftButton', () => {
  ...
  it('should be disabled when prevPage < 1', () => {
    vi.mocked(useParams).mockReturnValue({ details: undefined });

    render(<CardListLeftButton prevPage={0} />);
  ...
  it('should call pokemonNavigation with correct arguments on click when prevPage >= 1', async () => {
    const user = userEvent.setup();
    vi.mocked(useParams).mockReturnValue({ details: '1' });

    render(<CardListLeftButton prevPage={2} />);
    const button = screen.getByText('←');
